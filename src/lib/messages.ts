...
export class MessagesClass extends BaseClass {
                ...
                while (true) {
                    ...
                    for (let b = 1; temp.length > b; b++) {
                        ...
                        if (key[0] == '[') {
                            ...
                            if (
                                arraykey[1] &&
                                this.formatedData[arraykey[1] as keyof customFormatedKeysDef] !== undefined
                            ) {
                                const n = this.formatedData[arraykey[1] as keyof customFormatedKeysDef];

                                if (n != '' && !Number.isNaN(n)) {
                                    msg += arraykey[0]
                                        .split(',')
                                        [
                                            this.formatedData[arraykey[1] as keyof customFormatedKeysDef] as number
                                        ].trim();
                                }
                            } else {
                                this.log.error(
                            ...
                            key &&
                            this.formatedData[key.toLowerCase() as keyof customFormatedKeysDef] !== undefined
                        ) {
    ...
    async sendMessage(
        ...
        if (this.formatedData) this.formatedData.countdown = this.getCountdown(this.starttime);
